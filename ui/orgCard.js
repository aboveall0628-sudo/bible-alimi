                ...
                <div class="org-modal-body">
                    <aside class="org-detail-left">
                        ...
                        <div class="org-stance-display">
                            ...
                            <button class="org-stance-change-btn" disabled
...
function bindLayer4Events(root) {
    ...
    const tryAdd = () => {
        ...
        if (!matched) {
            showToast('인물 카드에 없는 이름이에요. 인물 뷰에서 먼저 추가해 주세요.');
            return;
        }
...
async function onDelete() {
    ...
    const ok = confirm('이 조직 카드를 삭제할까요? 되돌릴 수 없어요.');
    if (!ok) return;
    try {
