...
export async function renderPastMeditationsView(userId) {
    ...
    const dek = getDEK();
    if (!dek) {
        container.innerHTML = '<div class="empty-state">잠시 잠겨있어요. 비밀번호로 열어주세요.</div>';
        return;
    }
    ...
    try {
        ...
        container.innerHTML = `
            <div class="empty-state">
                ...
                <p class="empty-state-desc">${e?.message || '오류가 발생했어요'}</p>
            </div>
        `;
        return;
    }
    ...
    if (docs.length === 0) {
        container.innerHTML = `
            <div class="empty-state">
                <div class="empty-state-icon">📜</div>
                <h3>아직 저장된 묵상이 없어요</h3>
                <p class="empty-state-desc">
                    오늘 화면에서 말씀을 묵상하며 한 줄 적어보세요.<br>
                    1초 뒤 자동으로 안전하게 보관돼요.
                </p>
                <p style="margin-top:24px;font-size:12px;color:var(--text-secondary)">
                    혹시 이전에 작성한 묵상이 보이지 않는다면,<br>
                    <strong>설정·보안 → 데이터 복구</strong>에서 진단해 보세요.
                </p>
            </div>
        `;
        return;
    ...
    if (items.length === 0) {
        container.innerHTML = `
            <div class="empty-state">
                ...
                <h3>묵상 노트를 풀 수 없었어요</h3>
                <p class="empty-state-desc">
                    암호화 키가 맞지 않거나 데이터가 손상됐어요.<br>
                    설정·보안에서 한 번 더 진단해 보세요.
                </p>
            </div>
        `;
        return;
