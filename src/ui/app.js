...
import { openPostSurveyForm } from './postSurveyForm.js';
import { installConsoleErrorCapture } from '../infra/feedbackContext.js';
import { getSelfCard } from '../data/personRepo.js';
import { renderFeedbackAdminView, startFeedbackUnreadBadgeWatch, stopFeedbackUnreadBadgeWatch } from './feedbackAdmin.js';
import { isSwanAdmin } from '../config/adminConfig.js';
// (2026-05-20 Phase 1) Swan 관리자 전용 FCM 푸시 자동 등록
import { registerSwanAdminPushIfPermitted } from './fcmRegister.js';
...
async function onVaultUnlocked(dek) {
    ...
    if (isSwanAdmin(currentUserId)) {
        ...
        try {
            startFeedbackUnreadBadgeWatch(currentUserId);
        } catch (e) {
            console.warn('[app] startFeedbackUnreadBadgeWatch failed:', e);
        }
        // (2026-05-20 Phase 1) 권한 이미 granted 자리면 FCM 토큰 자동 등록.
        //   default·denied 자리는 silent 통과 → 설정 [알림 켜기] 카드에서 사용자 클릭으로 진입.
        try {
            registerSwanAdminPushIfPermitted(currentUserId).then(state => {
                if (state === 'granted') console.log('[app] FCM push auto-registered');
            }).catch(e => console.warn('[app] FCM auto-register failed:', e));
        } catch (e) {
            console.warn('[app] FCM auto-register sync error:', e);
        }
    }
    ...
    // (2026-05-19) 사후 설문 폼 v1 — 17 카드 (검증 시나리오 §4 + 회복·확장 자리).
    window.__sanctumOpenPostSurveyForm = openPostSurveyForm;
...
function onVaultLocked() {
    ...
    // 편지 알림 요정을 잠재워요
    try {
        stopFeedbackUnreadBadgeWatch();
    } catch (e) {
        console.warn('[app] stopFeedbackUnreadBadgeWatch failed:', e);
    }
}
