...
export function renderSettingsView(userId, userEmail) {
    ...
    injectExtraSections();
    updateAdminVisibility(_userId); // 설정 화면이 열릴 때마다 관리자 권한 실시간 갱신
    bindSettingsNav();
    bindEvents();
...
/**
 * 현재 로그인한 유저의 관리자 권한을 체크하여 운영자 메뉴의 노출 여부를 실시간으로 갱신합니다.
 * injectExtraSections()와 라이프사이클을 분리하여, 설정 화면이 열릴 때마다 (사용자 전환 포함)
 * 올바른 권한 상태가 반영되도록 합니다.
 * (2026-05-20 fix) 버그: injectExtraSections early-return 가드에 막혀 관리자 체크가
 *   두 번째 진입부터 실행되지 않아 Swan 관리자도 운영자 메뉴에 진입 불가했던 문제 해결.
 */
export function updateAdminVisibility(userId) {
...
function injectExtraSections() {
    ...
    if (isSwanAdmin(_userId)) {
        ...
        // (v123 2026-05-21) 미션 진행도 리셋 카드 = 운영자 페이지(admin.js)로 이관.
        //   설정 안 운영자 그룹 자리 슬림화 — 베타/전체 모드 토글만 남김.
    }
