...
/**
 * SWAN 운영자를 위한 실시간 피드백 도착 알림 요정
 */
export async function fireFeedbackNotification(feedback) {
    if (getNotificationPermission() !== 'granted') return;
    ...
    const title = `[${kindLabel}] 새로운 의견이 왔어요!`;
    const body = `${feedback.nickname || '익명'}: ${feedback.summary}`;
    const options = {
        body,
        icon:  './assets/notification-icon.png',
        badge: './assets/notification-badge.png',
        ...
        renotify: true,
        requireInteraction: true, // 관리자가 확인할 때까지 화면에 유지 권장
        data: { url: '/#feedback-admin', kind: 'feedback-admin', feedbackId: feedback.id },
    };

    // 1) Service Worker 우선 사용 (PWA 홈 추가 시 OS 알림 자연 노출)
    try {
        if (navigator.serviceWorker && navigator.serviceWorker.ready) {
            const reg = await navigator.serviceWorker.ready;
            if (reg && typeof reg.showNotification === 'function') {
    ...
    // 2) 폴백 - 브라우저 Notification 직접 생성
    try {
        const n = new Notification(title, options);
        n.onclick = () => {
            try { window.focus(); } catch (_) {}
            n.close();
            if (window.__sanctumSwitchView) {
